    ...
    <nav class="navbar navbar-expand-lg bg-white bg-opacity-75 backdrop-blur-sm sticky-top border-bottom border-light-subtle">
        <div class="container py-2">
            <a class="navbar-brand fw-bold d-flex align-items-center gap-2" href="index.jsp">
                <img src="assets/images/logo.jpg" alt="Complaint Portal logo" style="width:40px;height:40px;border-radius:12px;object-fit:cover;">
                <span>Complaint Portal</span>
            </a>
            <div class="ms-auto d-flex gap-2 flex-wrap">
                <a href="adminDashboard.jsp" class="btn btn-outline-primary">Admin Dashboard</a>
                <a href="officerDashboard.jsp" class="btn btn-outline-primary">Officer Dashboard</a>
    ...
    <main class="container py-4 py-lg-5">
        ...
        <div class="dashboard-card p-3 p-lg-4 mb-4">
            <form method="get" class="row g-3 align-items-end">
                ...
                <div class="col-md-3 d-flex flex-wrap gap-2">
                    <button type="submit" class="btn btn-primary" aria-label="View analytics for selected filters">View</button>
                    <a class="btn btn-warning" href="<%= base %>/actions/SendPendingReminders.jsp?date=<%= ("day".equals(g)? selectedDate : ("month".equals(g)? selectedMonth+"-01" : selectedYear+"-01-01")) %>">Send reminders</a>
                    <a class="btn btn-danger" href="<%= base %>/actions/ArchiveSolved.jsp?g=<%= g %><%= "day".equals(g)?("&date="+selectedDate):( "month".equals(g)? ("&month="+selectedMonth) : ("&year="+selectedYear) ) %>">Archive solved</a>
                </div>
            </form>
        </div>
        ...
        <div class="dashboard-card p-3 p-lg-4">
            <div class="d-flex flex-wrap gap-2 align-items-center">
                ...
                <a class="btn btn-outline-warning" href="<%= base %>/actions/SearchComplaints.jsp?date=<%= selectedDate %>&status=Pending">Pending: <%= pending %></a>
                <a class="btn btn-outline-info" href="<%= base %>/actions/SearchComplaints.jsp?date=<%= selectedDate %>&status=Solving">Solving: <%= solving %></a>
                <a class="btn btn-outline-success" href="<%= base %>/actions/SearchComplaints.jsp?date=<%= selectedDate %>&status=Solved">Solved: <%= solved %></a>
                <% if (selectedCategory != null && !selectedCategory.isBlank()) { %>
                    <div class="card p-3" style="border-left: 4px solid #3b82f6;"><b>Category:</b> <%= selectedCategory %><br><span>Pending: <b><%= catPending %></b></span><br><span>Solved: <b><%= catSolved %></b></span><br><div class="mt-2 d-flex gap-2"><a class="btn btn-sm btn-outline-warning" href="<%= base %>/actions/SearchComplaints.jsp?date=<%= selectedDate %>&status=Pending&category=<%= selectedCategory %>">View Pending</a><a class="btn btn-sm btn-outline-success" href="<%= base %>/actions/SearchComplaints.jsp?date=<%= selectedDate %>&status=Solved&category=<%= selectedCategory %>">View Solved</a></div></div>
                <% } %>
            </div>
        </div>
        ...
        <div class="d-flex flex-wrap gap-2 mt-4">
            ...
            <a href="<%= base %>/index.jsp" class="btn btn-outline-secondary">Back to Home</a>
